         ...
         var initExperiment = function () {
		  ...
		  console.log( $scope.idToGroup);
                  ...
                  if ($scope.config.hasOwnProperty("input_addresses")) {
                     // at this point, the config locations should be made
			   ...
			   $scope.input_array[subjectNum] = [];
                           ...
                           $scope.input_array[subjectNum] = $http.get(input_addresses[subjectNum-1]).then(function (response){
			      ...
			      var rows = response.data.split("\n");                    //split csv up line by line into an array of rows
			         ...
			         single_input_array[i] = [];
                                 ...
                                 for (let j = 0; j < cells.length; j++) {              //for each column in csv ro
                                    if(j == 1) {
                     ...
                     setTimeout(function() { console.log("After timeout"); }, 5000);
