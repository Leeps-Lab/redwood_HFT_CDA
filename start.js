   ...
   ["$scope",
      ...
      function ($scope, rs, dataHistory, graphing, $http) {
         ...
         rs.recv("Experiment_Begin", function (uid, data) {
            ...
            if (data.hasOwnProperty("input_addresses")) {
               ...
               $http.get(data.input_addresses[index]).then(function (response) {
                  ...
                  var delay = $scope.inputData[0][0];   //use player time offset w/o delay calculation
         ...
         $scope.processInputAction = function (inputIndex) {
            ...
            //delay
            var delay = $scope.inputData[inputIndex + 1][0] + $scope.inputData[inputIndex][0]; //delay calculation is next timestamp - current timestamp w/o offset
            console.log("delay: " + delay);
